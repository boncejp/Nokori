...
describe("validateTransactionPayload", () => {
  describe("正常系", () => {
    ...
    it("メモの前後空白は trim される", () => {
      const result = validateTransactionPayload({ amount: 500, kind: "NORMAL", memo: "  スーパー  " });
      expect(result.success).toBe(true);
      if (!result.success) return;
      expect(result.data.memo).toBe("スーパー");
    });

    it("メモがちょうど 200 文字のとき成功する", () => {
      const result = validateTransactionPayload({ amount: 500, kind: "NORMAL", memo: "あ".repeat(200) });
      expect(result.success).toBe(true);
    });
  });
  ...
  describe("金額のバリデーション", () => {
    ...
    it("金額が負数のとき失敗する", () => {
      const result = validateTransactionPayload({ amount: -1, kind: "NORMAL" });
      expect(result.success).toBe(false);
    });
  ...
  describe("メモのバリデーション", () => {
    it("メモが 201 文字のとき失敗する", () => {
      const result = validateTransactionPayload({ amount: 500, kind: "NORMAL", memo: "あ".repeat(201) });
      expect(result.success).toBe(false);
    });
  ...
  describe("種別と utilityType の組み合わせ制約", () => {
    it("SPECIAL かつ utilityType が指定されているとき失敗する", () => {
      const result = validateTransactionPayload({ amount: 10_000, kind: "SPECIAL", utilityType: "ELECTRICITY" });
      expect(result.success).toBe(false);
    });
    ...
    it("NORMAL かつ utilityType が指定されているとき失敗する", () => {
      const result = validateTransactionPayload({ amount: 5_000, kind: "NORMAL", utilityType: "GAS" });
      expect(result.success).toBe(false);
    });
  });
