...
| `monthly_income` | int | 月収（手取り）。オンボ初回保存では **0** とし、通常サイクル以降は給料日モーダルや設定で更新する |
...
実装では、**現在の論理日が「初回サイクル」に含まれるか**を先に判定し、分岐する。判定には `isWithinFirstCycle` を用いる。

- **初回サイクル:** `target_anchor_logical_date`（オンボーディング完了の論理日）から、**次の給料日の前日**（論理日ベース）まで。
- **通常サイクル:** 上記期間外、かつ初回サイクル締め処理（次の給料日リセット）完了後。

**27:00 との整合:** 論理日キーがアンカー暦日より 1 日手前になるのは、JST でアンカー暦日の 0:00〜2:59（未明）のみである。この帯では **フェーズ判定** に限り、同一瞬間の実時刻（`wallClockNow`）がアンカー暦日かつ 3 時未満なら初回サイクル内とみなす（ダッシュボード・設定 PATCH・取引 API 等で `getLogicalDate` と同じ `now` を渡す）。月次リセットの締め区間など、`referenceDate` が「昨日」等になる呼び出しでは `wallClockNow` を渡さず、論理日キーのみで判定する。
